...
from app.queries import locking_check, db_update, make_tx, to_checksum, main_process
...
def transaction_request():
    if RATE_LIMIT_ENABLED:
        ...
        # else:
        #     return make_response(incorrect_address(checksum_response['error_message']))
        #
        # is_address_allowed = locking_check(DucatusAddress, duc_address)
        # if not is_address_allowed:
        #     return make_response(duc_address_locked())
        #
        # ip = request.headers.getlist("X-Forwarded-For")[0] if request.headers.getlist(
        #     "X-Forwarded-For") else request.remote_addr
        #
        ...
        #     return make_response(ip_address_locked())
        #
        # tx_response = make_tx(duc_address)
        # if not tx_response['success']:
        #     return make_response(tx_failed(tx_response['error_message']))
        #
